...
// ---------------- CHECK INSTALLED ----------------
static int check_installed(const char* title_id, int timeout) {
    sqlite3* db = NULL;
    sqlite3_stmt* stmt = NULL;
    int ret = -1;
...
// ---------------- GET TITLE_ID ----------------
static int get_title_id(const char* app_path, char* title_id, size_t size) {
    char path[PATH_MAX];

    // Try param.json first (PS5)
    snprintf(path, sizeof(path), "%s/sce_sys/param.json", app_path);
    FILE* f = fopen(path, "rb");
    if (f) {
        fseek(f, 0, SEEK_END);
        long len = ftell(f);
    ...
    snprintf(path, sizeof(path), "%s/sce_sys/param.sfo", app_path);
...
static int find_mountdev(const char* file_path, char* dev_path, size_t dev_path_len) {
...
int main(int argc, const char* argv[]) {
    ...
    snprintf(param_json_path, sizeof(param_json_path),
             "%s/sce_sys/param.json", system_ex_app);
    ...
    if (check_installed(title_id, 5) || update_snd0info(title_id))
        notify("Failed to update sound");
